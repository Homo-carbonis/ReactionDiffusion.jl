...
timeseries_plot(model,params0; dt=0.5, tspan=tmax, noise=0.01, max_attempts=0, num_verts=64, normalise=false, hide_y=false, species=[:M])
...
function isvalid(sol)
    ...
    tvalid = t < tmax
    @show [maxvalid, minvalid, midvalid, quartervalid, tvalid]
...
params = rand(params, 100)
sol = simulate(model,params; dt=1.0, tspan=5e5)
