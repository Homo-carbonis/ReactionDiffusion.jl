...
map_params = product(k_Bmp=range(0.01,10.0, 20), k_Admp = range(0.01,10.0, 20), λ_Chd = [0.1], λ_ChdBmp = [0.1], λ_ChdAdmp = [0.1], η_Chd = [10^1.5], Xlr = [0.01], D_Chd = [1.0], D_Lig = range(0.01,10,20), D_Comp = range(0.01,10,20), L=logrange(10.0,1000.0,100))


p,c = plot_cost(model, cost, vars,map_params; maxrepeats=0)
heatmap(a, b, c, colormap=:redblue, colorrange=(0.0,1.0))
# optimise(model, cost, vars, params; η=0.1, sample=sample, dt=0.001, tspan=1000.0, num_verts=n)
